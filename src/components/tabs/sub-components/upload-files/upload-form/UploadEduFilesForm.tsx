...
const UploadEduFilesForm = ({
  ...
  const onSubmit = (data: TSubmittedData) => {
    ...
    if (classId) {
      setLoading(true);
      dispatch(actSendEduUploadedFiles({ classId, data }))
        .unwrap()
        .then((res) => {
    ...
    <form method="post" onSubmit={handleSubmit(onSubmit)}>
      ...
      {loading ? (
        ...
        <>
          <div
            className={
              previewFiles.length > 0 ? upload_box_holds_files : upload_box
            }
