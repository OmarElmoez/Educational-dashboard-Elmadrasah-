...
import {
  actGetReviewQuestions,
  actPostReviewAnswers,
} from "@/store/review-questions/reviewSlice";
...
const ReviewForm = forwardRef(({ lesson_id }: TModalProps, ref) => {
  ...
  const { records, loading } = useAppSelector((state) => state.reviewQuestions);
  ...
  return createPortal(
    <>
      <dialog
        className={`${reviewForm} modal py-[2.4rem] px-[1.6rem]`}
        ref={dialog}
      >
        <header>
          <div>
            <h1 className="text-[2rem] text-black font-medium">
              {credintials?.role === "Teacher"
                ? "تقييم الطالب"
                : "تقييم المعلم"}
            </h1>
            <p className="mt-[1rem] text-[#A0A1A7] text-[1.4rem] font-base">
              إعطاء تعليق علي مستوي{" "}
              {credintials?.role === "Teacher" ? "الطالب" : "المٌعلم"} خلال
              السيشن
            </p>
          </div>
