...
const OtpCode = () => {
  ...
  const resendOTP = async () => {
    setPending();
    await ResetPasswordServices.resendOtp(state.data).then((res) => {
      if (res?.status === 200) {
        setSucceeded();
      }
    });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <h2 className="text-black font-medium text-[2rem] mt-[4rem] text-center">
        يرجى التحقق من البريد الإلكتروني الخاص بك
      </h2>
      <p className="text-[1.6rem] mt-[3rem] text-center text-black">
        لقد أرسلنا رمزًا إلى {state.data.email}
      </p>
      <Controller
        name="otp"
        control={control}
        render={({ field }) => (
          <>
            ...
            />
            {errors.otp && (
              <p className="text-red-500 text-[1.4rem] mt-2 mb-10 text-center">
                {errors.otp.message}
              </p>
            )}
          </>
      ...
      >
        {isPending ? "برجاء الانتظار..." : "تاكيد"}
      </Button>
    </form>
  );
};
